...
{
    //Info << "use standard for chemistry" << endl;
    //BasicChemistryModel<ReactionThermo>::correct();
    ...
    forAll(curRho, celli)
    {
        ...
        if (Ti > Treact_)
        {
           ...
           // scalar pi = p[celli];
            ...
            for (label i=0; i<nSpecie_; i++)
            {
                c_[i] = rhoi*curY[i][celli]/specieThermo_[i].W();
                c0[i] = c_[i];
            }
            ...
            {
                RR_[i][celli] =
                    (c_[i] - c0[i])*specieThermo_[i].W()/deltaT;
            }
        }
        else
        {
...
const scalarField& curRho,
const PtrList<scalarField>& curY,
List<scalarField>& curRR,
scalarField& timeOfOde
)
{
    scalar deltaTMin = GREAT;
    ...
    forAll(curRho, celli)
    {
        ...
        if (Ti > Treact_)
        {
           ...
           // scalar pi = p[celli];
            ...
            for (label i=0; i<nSpecie_; i++)
            {
                c[i] = rhoi*curY[i][celli]/specieThermo_[i].W();
                c0[i] = c[i];					
            }
			...
			scalar solveChemistryCpuTime = 0;
            // Calculate the chemical source terms
            while (timeLeft > SMALL)
            {
                scalar dt = timeLeft;
            ...
            solveChemistryCpuTime  += timeIncr;
			timeOfOde[celli] = solveChemistryCpuTime;
            ...
            {
                curRR[i][celli] =
                    (c[i] - c0[i])*specieThermo_[i].W()/deltaT;
            }
		}
		else
        {
