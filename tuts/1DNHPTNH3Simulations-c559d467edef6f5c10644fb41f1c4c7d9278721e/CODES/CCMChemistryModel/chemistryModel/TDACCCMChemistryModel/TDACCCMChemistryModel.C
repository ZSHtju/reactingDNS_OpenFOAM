...
Foam::TDACCCMChemistryModel<ReactionThermo, ThermoType>::TDACCCMChemistryModel
(
...
// * * * * * * * * * * * * * * * * Destructor  * * * * * * * * * * * * * * * //
template<class ReactionThermo, class ThermoType>
Foam::TDACCCMChemistryModel<ReactionThermo, ThermoType>::
...
template<class ReactionThermo, class ThermoType>
Foam::scalar Foam::TDACCCMChemistryModel<ReactionThermo, ThermoType>::solveCCM
(
    const scalar deltaT,
    const label nBeg,
    const label nLast,
...
//***
    ...
    scalarField c0(this->nSpecie_);
        ...
        //Pout<<"nLast==="<<nLast<<endl;
        //Pout<<"FieldCCM[0].size()==="<<FieldCCM[0].size()<<endl;
    ...
    for(label icell=nBeg; icell < nLast; icell ++)
    {
        ...
        for (label i=0; i<this->nSpecie_; i++)
        {
            ...
            c0[i] = c[i];
            phiq[i] = FieldCCM[i+5][celli]; // for TDAC
        }
// for TDAC
...
//***
        ...
        else
        {
            ...
            if (reduced)
            {
                ...
                this->mechRed_->reduceMechanism(c, Ti, pi);
                nActiveSpecies += this->mechRed_->NsSimp();
                nAvg++;
            ...
            while (timeLeft > small)
            {
                ...
                if (reduced)
                {
                    ...
                    this->solve
                    (
                        this->simplifiedC_, Ti, pi, dt, deltaTChemIni
                    );

                    for (label i=0; i<this->NsDAC_; i++)
                    {
                        //c[this->simplifiedToCompleteIndex_[i]] = this->simplifiedC_[i];
                        c[this->simplifiedToCompleteIndex()[i]] = this->simplifiedC_[i];
                    }
                }
                else
                {
            ...
            if (this->tabulation_->active())
            {
                ...
                label growOrAdd =
                    this->tabulation_->add(phiq, Rphiq, rhoi, deltaT);
                if (growOrAdd)
                {
            ...
            if (reduced)
            {
                this->nSpecie_ = this->mechRed_->nSpecie();
            }
            deltaTMin = min(deltaTChemIni, deltaTMin);
        ...
        {
            RRCCM[i][celli] =
                (c[i] - c0[i])*this->specieThermo_[i].W()/deltaT/rhoi; // /rhoi Shenghui 20200302
        }
    }
...
// ************************************************************************* //
template<class ReactionThermo, class ThermoType>
void Foam::TDACCCMChemistryModel<ReactionThermo, ThermoType>::readCCMProperties()
{
...
template<class ReactionThermo, class ThermoType>
Foam::scalar   Foam::TDACCCMChemistryModel<ReactionThermo, ThermoType>::CCMIndexMapSolve
(
const scalar deltaT,
...
/*
	forAll(this->Y_,i)
	{
			for(label izone = nBeg; izone < nLast; izone++)
			{
				if(gFieldCCM[2][izone] != 0)
				{
				  gRRCCM[i][izone]  = gRRCCM[i][izone]/gFieldCCM[2][izone];
				}
				else
				{
				  gRRCCM[i][izone]  = 0;
