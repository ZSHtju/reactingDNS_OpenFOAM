...
(
const scalar deltaT,
const scalarField& curP,
const PtrList<scalarField>& curY,
const scalarField& curRho,
const scalarField& curT
)
{
// for TDAC
    // Increment counter of time-step
...
//***
    ...
    forAll(curRho, celli)
    {
        scalar Ti = curT[celli];
        scalar pi = curP[celli];
        const scalar rhoi = curRho[celli];

        for (label i=0; i<this->nSpecie_; i++)
        {
            c[i] = rhoi*curY[i][celli]/this->specieThermo_[i].W();
            c0[i] = c[i];
            phiq[i] = curY[i][celli]; // for TDAC
        }
// for TDAC
        phiq[this->nSpecie()]=Ti;
        phiq[this->nSpecie() + 1]=pi;
